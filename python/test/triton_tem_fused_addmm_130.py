...
@triton.testing.perf_report(
    triton.testing.Benchmark(
        ...
        line_arg="provider",
        line_vals=["torch", "baseline"],
        line_names=["Torch", "Baseline"],
        styles=[("green", "-"), ("blue", "-")],
        ylabel="TFLOPS",
...
def benchmark(m: int, n: int, k: int, provider: str):
    ...
    if provider == "torch":
        ms, min_ms, max_ms = triton.testing.do_bench(lambda: torch_tem_fused_addmm_130(input, a, b),
                                                     quantiles=quantiles)
    if provider == "baseline":
        ms, min_ms, max_ms = triton.testing.do_bench(lambda: triton_tem_fused_addmm_130(input, a, b, out_triton),
                                                     quantiles=quantiles)
...
def test_triton_tem_fused_addmm_130_kernel(m: int, n: int, k: int):
    ...
    # Using highest `rtol` and `atol` from `tune_gemm.py`.
